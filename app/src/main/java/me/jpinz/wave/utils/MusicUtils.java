...
import me.jpinz.wave.loaders.FavoritesLoader;
import me.jpinz.wave.loaders.LastAddedLoader;
import me.jpinz.wave.loaders.PlaylistLoader;
...
public class MusicUtils {
    ...
    public final static long[] getSongListForFavoritesCursor(Cursor cursor) {
        ...
        int colidx = -1;
        try {
            colidx = cursor.getColumnIndexOrThrow(FavoritesStore.FavoriteColumns.ID);
        } catch (final Exception ignored) {
        }
        for (int i = 0; i < len; i++) {
